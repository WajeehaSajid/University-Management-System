# University Management System

**Data Structures (DS) Project**

---

## Description

A comprehensive **University Management System** built in C++ using core Data Structures concepts. Manages students, courses, faculty, and academic records through an interactive console interface.

---

## Data Structures Used

- Linked Lists — student and course records
- Stacks / Queues — scheduling and waitlists
- Trees — hierarchical department structure
- Sorting & Searching algorithms

---

## Features

- Student registration and record management
- Course enrollment and management
- Faculty assignment
- Grade and result tracking
- Search and sort functionality across records

---

## How to Compile & Run

### Requirements
- g++ compiler or any C++ IDE (Visual Studio, Code::Blocks, Dev-C++)

### Compile
```bash
g++ Uni_managment_system_DS_Project.cpp -o uni_system.exe
```

### Run
```bash
uni_system.exe
```

Or open the `.cpp` file in your IDE and press Build & Run.

---

## Tech Stack

- C++
- Standard Template Library (STL)
- Console-based UI

---

## License

MIT License
